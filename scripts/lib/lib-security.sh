...
# Vendored from: loa-constructs/.claude/scripts/lib-security.sh @ df1c0304
# Adaptations:
#   - Config lookup key changed from `flatline_protocol.secret_scanning.patterns`
#     to `codex_review.secret_patterns` (decoupled from bonfire/Loa config)
#
# Used by:
#   - codex-review-api.sh (auth + redaction)
...
_build_redaction_sed() {
  ...
  if [[ -f "$config_file" ]] && command -v yq &>/dev/null; then
    ...
    extra_patterns=$(yq eval '.codex_review.secret_patterns[]? // empty' "$config_file" 2>/dev/null) || true
...
_redact_json() {
  ...
  if ! redacted=$(echo "$content" | jq "$jq_filter" 2>/dev/null) || [[ -z "$redacted" ]]; then
    echo "[codex-review-security] ERROR: jq redaction failed, returning original" >&2
    echo "$content"
    return 1
  fi
  ...
  if ! echo "$redacted" | jq empty 2>/dev/null; then
    echo "[codex-review-security] ERROR: Redaction produced invalid JSON" >&2
    echo "$content"
    return 1
  fi
  ...
  if [[ "$pre_key_count" != "$post_key_count" ]]; then
    echo "[codex-review-security] ERROR: Redaction changed key count ($pre_key_count → $post_key_count)" >&2
    echo "$content"
    return 1
  fi
